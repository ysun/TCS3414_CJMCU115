...
//////ysun//////
#define AILTL 0xa4 // register address + command bits
#define AILTH 0xa5 // register address + command bits
#define AIHTL 0xa6 // register address + command bits
#define AIHTH 0xa7 // register address + command bits
...
void init_TCS34725(void)
{
  ...
  //ysun i2cWriteBuffer[0] = 0x00;
  i2cWriteBuffer[0] = 0x2;
...
/////ysun////
  i2cWriteBuffer[0] = 0x13;
  Writei2cRegisters(1,EnableAddress);    // enable ADs and oscillator for sensor 
  i2cWriteBuffer[0] = 0x0;
  Writei2cRegisters(1,AIHTH);
  i2cWriteBuffer[0] = 0x8f;
  Writei2cRegisters(1,AIHTL);
}
...
void setup() {
  ...
  attachInterrupt(1, func_interrupt, CHANGE);
...
void loop() {
	...
	if (1) {
		get_Colors();
		Wire.beginTransmission(SensorAddressWrite);   // Send address with Write bit set
		Wire.write(0x66);                          // Send command, normally the register address 
		Wire.endTransmission();
		trigger_int = false;
	}
